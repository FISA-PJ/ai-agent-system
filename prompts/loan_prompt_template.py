...
3. 필터링 된 대출 상품들 중에서 사용자에게 추천할 상품 3개를 다음 형식에 맞춰 표로 정리하세요:
   - 정리한 표에는 다음 항목만 포함하고, 각 상품은 깔끔하게 나열해주세요:
        - 은행명
        - 상품명
        - 금리 범위 (최저 ~ 최고)
        - 대출 한도 (억 단위)
        - 상환 기간
        - 상환 방식
        - 추천 이유: 사용자 정보와 어떤 점에서 잘 맞는지 설명

   - 마지막에는 "자세한 대출 정보는 해당 은행 홈페이지를 참고하시기 바랍니다."라는 문장을 반드시 추가하세요.
...
# react_loan_agent_notice에에 사용되는 프롬프트
...
5. 필터링 된 대출 상품들 중에서 사용자에게 추천할 상품 3개를 다음 형식에 맞춰 표로 정리하세요:
   - 정리한 표에는 다음 항목만 포함하고, 각 상품은 깔끔하게 나열해주세요:
        - 은행명
        - 상품명
        - 월 예상 상환액
        ...
        - 상환 방식
        - 추천 이유: 사용자 정보와 어떤 점에서 잘 맞는지 설명
   - 월 예상 상환액은 평균 금리를 기준으로 계산되었음을 안내하세요.
   - 마지막에는 "자세한 대출 정보는 해당 은행 홈페이지를 참고하시기 바랍니다."라는 문장을 반드시 추가하세요.
